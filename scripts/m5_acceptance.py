"""The M5 offline acceptance harness (Issue #89 PR-F §A, kickoff 5750366733).
    ...
    python scripts/m5_acceptance.py --verify <report.json>
...
code must run from a clean checkout of exactly one commit: no tracked change, no untracked file
and no ignored source. The run's application data lives under ``<root>/data``. The sanitized
report is written to ``<root>/m5-acceptance-report.json`` and printed. The exit status is 0 only
when the report has no problem, 1 when it has one, and 2 when the root or the checkout was refused
before anything was created.

The run is offline, synthetic and provider-zero: CREATE, image upload and product search are
NOT_ADOPTED, no marketplace mutation is reachable, and no approval phrase exists or is needed.
``--verify`` recomputes a report's digest.
"""
...
from scripts.m5accept.harness import run_acceptance
from scripts.m5accept.root import REPORT, RootRefused
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="m5_acceptance")
