...
| ReviewItem | the `ReviewKind` contract and the screen counters; the Gate 2 owner contract, ADR-0016 (G2-0); **G2-A:** the durable ReviewItem owner (migration 0021) with server-computed keys, the reconciliation-only lifecycle, audited transitions and owner-reconciled human resolution; **G2-B:** the COLLECT / M3 producer, its fast path, its startup and bounded periodic full reconciliation with a coverage watermark (migration 0022), and its review path on the COLLECT screen | M4 and REGISTER conditions are not indexed yet (G2-C); `open_counts()` still returns zero for every kind, and durable counts with `NOT_WIRED` and current-coverage semantics are G2-C |
