...
| LIVE authorization | the two-mode execution contract, `DRY_RUN` and `LIVE`; **the bounded LIVE grant and protected-write brake contract, ADR-0018 (G3-0)** | the grant, brake, ASSET upload-attempt owner and send-time stack exist provider-zero (Gate 3 area 1), but the execution-mode owner still enforces `M0_DRY_RUN_ONLY` and denies LIVE with `M0_LIVE_FORBIDDEN`, so the stack refuses every mutation |
