...
| ReviewItem | the `ReviewKind` contract and the screen counters; the Gate 2 owner contract, ADR-0016 (G2-0) | no table, no producer and no persistence; `open_counts()` returns zero for every kind although M3 already produces `REVIEW_REQUIRED` truth; the owner is Gate 2 (G2-A to G2-C), not yet implemented |
