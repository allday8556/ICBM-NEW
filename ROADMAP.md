...
| ReviewItem | the `ReviewKind` contract and the screen counters; the Gate 2 owner contract, ADR-0016 (G2-0); **G2-A:** the durable ReviewItem owner (migration 0021) with server-computed keys, the reconciliation-only lifecycle, audited transitions and owner-reconciled human resolution; **G2-B:** the COLLECT / M3 producer, its fast path, its startup and bounded periodic full reconciliation with a coverage watermark (migration 0022), and its review path on the COLLECT screen; **G2-C:** the M4 base-readiness and REGISTER producers (UNKNOWN, read-back MISMATCH, PAUSED scope), each with the same startup and periodic recovery, an owner truth token stored with every watermark and compared on every read (migration 0023), and `open_counts()` from durable OPEN rows per kind as `CURRENT`, `NOT_CURRENT` or `NOT_WIRED` over every producer that can emit the kind, on the dashboard and 품절 | STOCK, SOURCE_CHANGE, COLLECT_EVIDENCE, COMPLIANCE and FULFILLMENT stay `NOT_WIRED`: OPERATE's stock workflow, source drift, a collection run that ended with no revision (it has no ReviewItem scope under ADR-0016 §8), ComplianceGate and M6 have no producer; the M4 and REGISTER items are counted but not yet listed on the Product DB or Registration screens |
