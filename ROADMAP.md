...
3. **Gate 2 — the human review path** (Issue #89 kickoff `5804605624`; contract `docs/adr/0016-gate2-human-review-path-and-review-item-owner.md`). It gives `ReviewItem` a durable owner that indexes review conditions production owners already derive, never a second truth, one separately authorized slice at a time: G2-A the owner, its migration and audited lifecycle, G2-B the COLLECT / M3 producer, G2-C the M4 and REGISTER (execution and preparation) indexing with durable counts and `NOT_WIRED` semantics. `ComplianceGate`, LIVE authorization and M6 stay out. **Gate 2 is accepted** (GPT `5818393660`, independent cross-audit `5818648647`) on the exact-main closeout of `c031f5ad` (evidence `5818068828`; exact-main CI `35987718944`, 6/6); that evidence is not M5 acceptance. At Gate 2 close:
   - `REGISTRATION_ERROR` is the only kind that can carry an authoritative count, because both of its emitters (`register.execution`, `register.preflight`) are wired;
   - `COLLECT_EVIDENCE`, `STOCK`, `SOURCE_CHANGE`, `COMPLIANCE` and `FULFILLMENT` stay `NOT_WIRED`, and a wired kind whose coverage is not current is `NOT_CURRENT`; neither carries a count, so neither is ever a false zero;
   - `STOCK` is never authoritative on COLLECT coverage alone while `operate.stock` has no producer.
4. **Gate 3 — pre-LIVE safety and bounded LIVE authorization** (Issue #89 kickoff `5821078540`; contract `docs/adr/0018-gate3-pre-live-safety-and-bounded-live-authorization.md`). It exists only because M5 is still `PENDING`: the contract fixes the bounded, audited LIVE grant, the server-owned protected-write brake, canary eligibility without a ComplianceGate (the first canary is non-regulated only; ComplianceGate stays a separate later owner), and three proven prerequisites — a backup and restore drill, end-to-end evidence retention and a populated visual/responsive acceptance. The provider-evidence verdict stays an independent hard blocker: while CREATE and SEARCH are `NOT_ADOPTED`, the canary stays `BLOCKED` whatever else is proven. G3-0 is contract only; each later slice needs its own authorization, one at a time. M6 does not start before M5 acceptance.
...
| ReviewItem | the `ReviewKind` contract and the screen counters; the Gate 2 owner contract, ADR-0016 (G2-0); **G2-A:** the durable ReviewItem owner (migration 0021) with server-computed keys, the reconciliation-only lifecycle, audited transitions and owner-reconciled human resolution; **G2-B:** the COLLECT / M3 producer, its fast path, its startup and bounded periodic full reconciliation with a coverage watermark (migration 0022), and its review path on the COLLECT screen; **G2-C:** the M4 base-readiness producer, the REGISTER execution producer (UNKNOWN, read-back MISMATCH, PAUSED scope) and the REGISTER preparation producer (each current durable preparation's candidate preflight, derived by the existing preflight owner), each with the same startup and periodic recovery, an owner truth token stored with every watermark and compared on every read (migration 0023), and `open_counts()` from durable OPEN rows per kind as `CURRENT`, `NOT_CURRENT` or `NOT_WIRED` over every producer that can emit the kind, on the dashboard and 품절; each owner screen lists its own scope's items with its producers' coverage (수집관리, 통합DB, 등록관리) | STOCK, SOURCE_CHANGE, COLLECT_EVIDENCE, COMPLIANCE and FULFILLMENT stay `NOT_WIRED`: OPERATE's stock workflow, source drift, ComplianceGate and M6 have no producer, and a collection run that ended `NO_REVISION` stays an unwired potential emitter — no ADR defines a run-scoped ReviewItem identity or how its terminal condition would clear, so no item and no scope key exist for it (architect decision on PR #109). Product-semantics carry-forward from the Gate 2 cross-audit (`5818648647`): the onboarding counts (supplier and marketplace connections, products) and review-count authority are separate inputs, but the dashboard's `NO_CONNECTIONS` EMPTY verdict is their conjunction — every onboarding count zero **and** every kind an authoritative zero — so while five kinds are `NOT_WIRED` it is unreachable and a fresh install shows `READY`; whether to separate them is an open product decision, not authorized code |
| ComplianceGate | the `PASS / REVIEW_REQUIRED / BLOCKED` contract (`docs/ARCHITECTURE.md` §7); Gate 3 (ADR-0018 §5) keeps it a separate later owner and restricts the first canary to a product proven non-regulated by its reviewed category metadata — an eligibility restriction, never a `COMPLIANCE PASS` | no production owner decides it; regulated categories may not be claimed as automatically registrable |
| LIVE authorization | the two-mode execution contract, `DRY_RUN` and `LIVE`; **the bounded LIVE grant and protected-write brake contract, ADR-0018 (G3-0)** | no implementation: the execution-mode owner still enforces `M0_DRY_RUN_ONLY` and denies LIVE with `M0_LIVE_FORBIDDEN`, and no grant or brake owner exists |

## 14.2 Preconditions for the first LIVE write

Before any real marketplace write, and independently of endpoint adoption:

- a **bounded LIVE authorization contract** (§14.1; decided in ADR-0018, not implemented) and the user's explicit scope approval;
...
- a **visual and responsive acceptance gate over populated UI state** (ADR-0018 §9). The current UI tests are functional wiring tests driven through Playwright; no visual-regression or responsive acceptance evidence over populated screens exists.
