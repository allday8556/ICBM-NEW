...
## 9. Later revision of the acceptance script (PR #9, M1)

PR #9 revised `scripts/m0_acceptance.py`. The M0 evidence above was produced by the original script at `2c2c671` and remains valid for that commit. The revision changes one check only, and only its expected values:

- **"migrated to head":** the check used to compare against the hard-coded `0001_m0_foundation`. It now reads the head from the repository's own Alembic scripts, without importing application code. M1 legitimately adds `0002_m1_supplier_connections`.
- **"canonical tables empty":** the check now covers every table, including tables added by later milestones such as `supplier_connections`. That makes it stricter.

CI's migration job was changed the same way (`alembic heads`). The revised script still runs 24 checks and passes 24/24 on the M1 code; see `docs/acceptance/M1.md`. To reproduce the original M0 run exactly, check out `2c2c671` as shown above.
