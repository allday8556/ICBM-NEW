...
Status: **SUBMITTED FOR ARCHITECT REVIEW** — M0 is not accepted until architect review (Issue #1)
Author: Claude Code
Issue: [allday8556/ICBM-NEW#1](https://github.com/allday8556/ICBM-NEW/issues/1) · Branch: `feat/infra-m0-foundation`
Code commit under test: `2c2c6713d62a819ed9dabc57bd269824a92a5cd5`
...
- **L8** — Canonical documents still name older prototypes: `CLAUDE.md` §11 (v28), `ROADMAP.md` §1.3 (v27), `ui/prototypes/README.md` (v28). Implementation followed `docs/UI_SOURCE_OF_TRUTH.md` and Issue #1 (v29); Claude did not edit canonical documents.

## 7. Questions for the architect (non-blocking for this review)

- **Q1** — Deliverable 13: with REGISTER EMPTY, the approved zero-data surface hides the registration-settings panel, so the three automation toggles are not rendered at all in M0. Is that acceptable, or should the panel be shown read-only beside the empty state?
- **Q2** — Besides protected actions, M0 also audits `JOB_DEAD_LETTERED` (automation stopped; human attention needed) and `DIAGNOSTIC_REQUEST`. Keep both in AuditEvent?
- **Q3** — Job states `QUEUED | RUNNING | RETRY_SCHEDULED | SUCCEEDED | DEAD` and the `job_attempts` history table (the evidence for “retries on schedule”) are M0 schema choices. Accept, or rename to match another convention?
- **Q4** — Please sync the stale prototype references listed in L8.

## 8. Reproduce

```powershell
git clone --branch feat/infra-m0-foundation https://github.com/allday8556/ICBM-NEW.git
cd ICBM-NEW
py -3.12 -m venv .venv
