...
| `DRY_RUN` / `LIVE` | the global external-write mode. `LIVE` is currently refused outright by the execution-mode owner (`M0_DRY_RUN_ONLY`). Its authorization contract is ADR-0018 (not implemented); the mode alone is never authority for a mutation (`docs/ARCHITECTURE.md` §13). |
...
| ASSET upload attempt | one durable, server-owned record of one image upload (ADR-0018 §3.4), required before any upload: started before transmission in the same atomic unit that consumes the ASSET grant budget, terminal once as `APPLIED_PROVEN`, `NOT_APPLIED_PROVEN` or `UPLOAD_UNKNOWN`; an unfinished one after a crash is `UPLOAD_UNKNOWN`, and only `APPLIED_PROVEN` yields a known provider asset. Its provenance (grant, preparation revision, candidate, profile, `derivation_id`, local artifact kind) is separate from its **replay-conflict key** — marketplace, canonical account, provider upload endpoint and contract identity, and the canonical provider-visible upload-request identity (outbound binary digest plus provider-visible parameters) — and a started, `UPLOAD_UNKNOWN` or `APPLIED_PROVEN` attempt blocks every fresh upload with that key whatever the provenance. No such owner exists yet. |
