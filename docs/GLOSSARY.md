...
| ASSET upload attempt | one durable, server-owned record of one image upload (ADR-0018 §3.4), required before any upload: started before transmission in the same atomic unit that consumes the ASSET grant budget, terminal once as `APPLIED_PROVEN`, `NOT_APPLIED_PROVEN` or `UPLOAD_UNKNOWN`; an unfinished one after a crash is `UPLOAD_UNKNOWN`, and only `APPLIED_PROVEN` yields a known provider asset. Its provenance (grant, preparation revision, candidate, profile) is separate from its **replay-conflict key** — marketplace, canonical account, upload endpoint and contract identity, exact outbound artifact identity — and a started or `UPLOAD_UNKNOWN` attempt blocks every new upload with that key whatever the provenance. No such owner exists yet. |
