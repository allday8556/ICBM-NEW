...
| LIVE grant | the only authority for a marketplace mutation (ADR-0018 §3): durable, audited and server-owned, bound to one marketplace, canonical account, operation and endpoint group and provider-listing unit, a finite window and a finite mutation budget. `EXPIRED`, `REVOKED` and `EXHAUSTED` are terminal. It decides no readiness, adoption, compliance or provider truth. |
