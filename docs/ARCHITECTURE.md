...
The Adaptive Collector contract is `docs/adr/0017-adaptive-collector-profile-extraction-and-shadow-validation.md` (Issue #110). It authorizes no implementation by itself. **Only its offline core (production slice P1, `app/collect/adaptive/`) and its profile and validation persistence owner (P2, `app/collect/adaptive_store/`, migration 0024) exist, and nothing in the runtime calls either**. P2 stores immutable EPR/PTR revisions, the append-only `DRAFT`/`SHADOW`/`RETIRED` designation log, local-only validation samples and validation runs; `VALIDATED` is derived, never stored, and a `SHADOW` designation alone permits no shadow run. There is no COLLECT wiring, no shadow runner, switch or store, and no `ProductFactsRevision` write, each of which needs its own authorized slice:
