...
**The ReviewItem owner exists** (G2-A: `app/review/owner.py`, migration 0021) **with the COLLECT / M3 producer** (G2-B: `app/review/collect_producer.py`, coverage watermark migration 0022). COLLECT's `REVIEW_REQUIRED` conditions on each source identity's current revision are indexed, reconciled at startup and periodically, and shown on the COLLECT screen's run focus with the coverage verdict. `ReviewService.open_counts()` still returns zero for every kind: durable counts and their `NOT_WIRED` semantics are G2-C. That is now a gap, not a consequence of having no review work: M3 is accepted and COLLECT already produces `REVIEW_REQUIRED` source truth, and M4/M5 readiness carries it further. Until the owner exists, a `REVIEW_REQUIRED` fact is visible only in the screen that derives it, and a dashboard review count of zero proves nothing.
